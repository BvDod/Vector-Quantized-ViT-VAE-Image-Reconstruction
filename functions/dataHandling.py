...
def get_dataset(dataset_name: str, print_stats = False, disable_augmentation = False):
    ...
    if dataset_name == "MNIST":
        ...
        train = torchvision.datasets.ImageFolder("./CRC/train/", transform=img_transforms)
        test = torchvision.datasets.ImageFolder("./CRC/val/", transform=transforms_val)
        from torch.utils.data import Subset
        class_to_idx = train.class_to_idx

        filtered_train = [i for i, (_, label) in enumerate(train.samples) if not (label == 7 or label == 8)]
        filtered_test = [i for i, (_, label) in enumerate(test.samples) if not (label == 7 or label == 8)]

        # Create a subset
        train = Subset(train, filtered_train)
        test = Subset(test, filtered_test)
